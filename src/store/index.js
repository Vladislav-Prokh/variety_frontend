...
const store = createStore({
    ...
    actions: {
        async authenticate(context, { email, password }) {
            try {
                const { data } = await axios.post('http://localhost:8086/signin', {
                    email,
                    password
                }, {
        ...
        async register(context,{
            ...
            try {
                const { data } = await axios.post('http://localhost:8086/registration', {
                    username,
                    email,
                    password
                });
        ...
        async GET_ALLPRODUCTS(context) {
            try {
                const { data } = await axios.get('http://localhost:8086/home');
        ...
        async GET_SPECIFICPRODUCT(context,id) {
            try {
                const { data } = await axios.get(`http://localhost:8086/products/id/${id.id}`)
        ...
        async GET_SEARCHEDPRODUCTS(context, { name }) {
            try {
                const { data } = await axios.get(`http://localhost:8086/products/${name}`);
        ...
        async GET_ALLUSERS(context) {
            try {  
               const { data } = await  axiosInstance.get(`http://localhost:8086/users/all`)
        ...
        async GET_SPECISICUSER(context,param) {
            try {  
               const { data } = await  axiosInstance.get(`http://localhost:8086/user/${param.name}`)
        ...
        async GET_ALLCATEGORIES(context){
            try {  
                const { data } = await  axios.get(`http://localhost:8086/category/getAll`)
        ...
        async GET_ALLORDERS(context){
            try{
                const {data}  = await axiosInstance('http://localhost:8086/orders/All');
